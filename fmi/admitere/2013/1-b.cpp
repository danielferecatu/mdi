...
void s( int n, int p ) {
    ...
    */
    ...
    int chunk = m/2;
    for( int k=1; k<=n; k++ ) {               // --> Complexitate O(n)

        for( int i=chunk; i<m; i+= chunk ) {  // --> Complexitate O(2^n)

            // Daca nu am sters la un pas anterior
            if( v[i]!=sters ) {
                // Stergem si afisam
                v[i] = sters;
                cout << k << " " << i << endl;
            ...
            // Daca am dat peste p
                ...
                // Stergem pozitia m (cazul special)
                v[m] = sters;
                cout << k << " " << m << endl;
            }
    ...
    // --> Complexitate totala O(n) * O(2^n) = O(2^n)
